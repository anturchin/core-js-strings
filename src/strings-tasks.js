...
function isPalindrome(str) {
  const palinrome = str
    ...
    .replace(/[^\w\s]|_/g, '');
  let a = 0;
  let b = palinrome.length - 1;
  for (let i = 0; i < palinrome.length; ) {
    if (palinrome[a] !== palinrome[b]) {
      return false;
    }
    i += 1;
    a += 1;
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < strArr.length; ) {
    if (strArr[i].length > longStr.length) {
      longStr = strArr[i];
    }
    i += 1;
  }
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arr.length; ) {
    strRevers = `${strRevers + arr[i].split('').reverse().join('')} `;
    i += 1;
  }
  return strRevers.trimEnd();
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; ) {
    if (str[i] === str[i].toUpperCase()) {
      invert += str[i].toLowerCase();
    } else {
      invert += str[i].toUpperCase();
    }
    i += 1;
  }
  return invert;
...
function extractNameFromTemplate(value) {
  return value.split(', ')[1].replace(/[^\w\s]|_/g, '');
}
